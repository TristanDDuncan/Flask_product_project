...
products_schema = Productschemas(many=True)
...
class ProductResource(Resource):
    ...
    def put(self, product_id):
        ...
        if 'description' in request.json:
            product_from_db.name = request.json['description']
        if 'price' in request.json:
            product_from_db.name = request.json['price']
        if 'inventory_quantity' in request.json:
            product_from_db.name = request.json['inventory_quantity']
            db.session.commit() 
            return product_schema.dump(product_from_db)
